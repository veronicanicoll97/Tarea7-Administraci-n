...
class CategoriaRepository {
    ...
    async listadoCategorias(log) {
        try {
            ...
            const listadoCategoria = 
                await pgClient.categorias.findMany({})

            if(listadoCategoria.length === 0)
                return new Array();
    ...
    async categoriaById(idCategoria, log) {
        try {
            ...
            const categoriaPorId = 
                await pgClient.categorias.findUnique({
                    where: { idCategoria }
                })
    ...
    async eliminarCategoria(idCategoria, log) {
        try {
            log.info(
                "Inicia el proceso de eliminar la categoria: " + 
                idCategoria
            );
    ...
    async actualizarCategoria(idCategoria, datos, log) {
        try {
            ...
            log.info(
                "Inicia el proceso de actualizar la categoria: " + 
                idCategoria
            );
